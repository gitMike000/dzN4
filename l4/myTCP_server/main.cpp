...
public:
  bool send_buffer(const std::vector<char> &buffer) {
    ...
    while (transmit_bytes_count != size) {
      auto result =
          send(client_sock_, &(buffer.data()[0]) + transmit_bytes_count,
...
int main(int argc, char const *argv[]) {
  ...
  while (run) {
    ...
    if (recv_len > 0) {
      ...
      getnameinfo(reinterpret_cast<sockaddr *>(&client_addr), client_len, hbuf,
                  sizeof(hbuf), nullptr, 0, NI_NAMEREQD);

      if (AF_INET == ((struct sockaddr_in *)&client_addr)->sin_family) {
        inet_ntop(client_addr.ss_family,
                  &(((struct sockaddr_in *)&client_addr)->sin_addr), ipbuff,
                  INET_ADDRSTRLEN);
      } else if (AF_INET6 ==
                 ((struct sockaddr_in6 *)&client_addr)->sin6_family) {
        inet_ntop(client_addr.ss_family,
                  &(((struct sockaddr_in6 *)&client_addr)->sin6_addr), ipbuff,
                  INET_ADDRSTRLEN);
      } else {
